...
\begin{enumerate}
   ...
   \item  Evaluate the line integral of the vector field \( \vec{F} = x\hat{x} + y\hat{y} \) along the closed path defined by the points $A(0,1)\rightarrow B(1,1)\rightarrow C(0,0)\rightarrow A(0,1)$.
    ...
    \item Consider the radial vector field of the form $\vec{V} = k r^{-p} \hat{r}$ in three-dimensional space where $k$ is a constant. Which specific value of $p$ ensures that the total flux through any enclosing concentric sphere remains constant, and that the space itself contains absolutely no sources or sinks away from the origin? Provide a detailed explanation of your reasoning, including the mathematical derivation that leads to your conclusion.


    \newpage
    \item Calculate the gradient of a scalar field $f=1/R$, where $R$ is the magnitude of $\vec{R}=\vec{r}-\vec{r}~'$, with respect to primed coordinates. Follow the steps below:
    \begin{itemize}
        \item[(i)] Express $1/R$ in terms of the Cartesian coordinates.
        \item[(ii)] Compute the gradient $\vec{\nabla}'(1/R)$ by finding the partial derivatives of $1/R$ with respect to $x'$, $y'$, and $z'$. Note: $\vec{\nabla}'$ denotes the gradient with respect to the primed coordinates. 
        \item[(iii)] Simplify the result and express the gradient  it in terms of $\hat{R}$ and $R$.
    \end{itemize}
